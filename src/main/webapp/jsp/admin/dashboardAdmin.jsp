...
<html>
	<head>
		...
		<script>
			...
			//new - panels layout is via the publisher so no longer need to update this every time the dashboard layout changes
			manager.admin.iframeSrc = "/publisher-manager/private/app/kibana/${user.token}#/dashboard/XXXX-Dashboard?embed=true&_g=(refreshInterval:(display:Off,pause:!f,value:0),time:(from:now-24h,interval:'1d',mode:quick,timezone:Asia%2FJerusalem,to:now))&_a=(filters:!(),options:(darkTheme:!f),panels:!(${user.dashboardVisualsLayout}),query:(query_string:(analyze_wildcard:!t,query:'*')),title:'Dashboard',uiState:(P-2:(spy:(mode:(fill:!f,name:!n)))))&token=${user.token}";
